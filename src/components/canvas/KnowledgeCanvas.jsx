...
const CurvedEdge = ({
  ...
  return (
    <>
      {selected && (
        <path d={pathD} fill="none" stroke={style?.stroke || 'var(--accent)'} strokeWidth={8} strokeOpacity={0.2} className="pointer-events-none" />
      )}
      <path
      ...
      {data?.isRunning && (
        <>
          <circle r="4" fill={style?.stroke || 'var(--accent)'}>
        ...
        <EdgeLabelRenderer>
          ...
          >
            <div
              ...
              style={{
                backgroundColor: labelBgStyle?.fill || 'var(--surface)',
                color: labelStyle?.fill || style?.stroke || 'var(--accent)',
  ...
  return (
    ...
    >
      ...
      <button onClick={onResetZoom} className="px-2 py-0.5 min-w-[50px] text-sm font-medium hover:bg-gray-100 rounded transition-colors"
        style={{ color: 'var(--text-muted)' }} title="重置为 100%">
        {percentage}%
      </button>
...
function QuickAddMenu({ x, y, onSelect, onClose }) {
  ...
  return (
    ...
    >
      <div className="px-3 py-1 text-xs font-medium mb-1" style={{ color: 'var(--text-faint)', borderBottom: '1px solid var(--border-subtle)', letterSpacing: '0.15em' }}>
        快速添加
      </div>
      {options.map((option) => (
        <button
      ...
      <div style={{ borderTop: '1px solid var(--border-subtle)' }} className="mt-1 pt-1">
        <button onClick={onClose} className="w-full px-4 py-1.5 text-left text-xs hover:bg-gray-50" style={{ color: 'var(--text-faint)' }}>
          取消 (Esc)
        </button>
      </div>
    </div>
...
function KnowledgeCanvasInner({
  ...
  return (
    <div
      ...
      style={{ backgroundColor: 'var(--surface)', cursor: isPanMode ? 'grab' : 'default' }}
    ...
    >
      ...
      >
        ...
        <Panel position="top-left">
          ...
          >
            <span className="text-xs" style={{ color: 'var(--text-faint)', letterSpacing: '0.15em' }}>
      ...
      {contextMenu.visible && (
        <>
          ...
          >
            <div className="px-3 py-1.5 text-xs font-medium flex items-center gap-2" style={{ color: 'var(--text-faint)', borderBottom: '1px solid var(--border-subtle)', letterSpacing: '0.1em' }}>
